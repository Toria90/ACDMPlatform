...
import {ERC20MintBurn} from "../typechain-types"
...
describe("ERC20MinterBurnable contract", () => {
    ...
    let erc20MinterBurnableContract : ERC20MintBurn;
    ...
    beforeEach(async () => {
        ...
        erc20MinterBurnableContract = (await factory.connect(deployer).deploy(name, symbol, decimals)) as ERC20MintBurn;
    });
    ...
    describe ("mint", () => {
        it("Shouldn't be possible mint to zero address", async () => {
            const mintAmount = 1;
            await expect(erc20MinterBurnableContract.mint(constants.AddressZero, mintAmount))
    ...
    describe("transfer", () => {
        it("Shouldn't be possible transfer to zero address", async () =>{
            ...
            const toAddress : string = constants.AddressZero;
    ...
    describe ("approve", () => {
        it("Shouldn't be possible to zero address", async () => {
            const amount = 1;
            await expect(erc20MinterBurnableContract.connect(accounts[0]).approve(constants.AddressZero, amount))
    ...
    describe("transferFrom", () => {
        ...
        it("Should transfer without approve if from equal sender", async () =>{
            const transferAmount : number = 1;
            const sender : SignerWithAddress = accounts[1];
            const recipient :SignerWithAddress = accounts[2];

            await erc20MinterBurnableContract.mint(sender.address, transferAmount);
            await erc20MinterBurnableContract.connect(sender).transferFrom(sender.address, recipient.address, transferAmount);
            expect(await erc20MinterBurnableContract.balanceOf(sender.address)).to.equal(0);
        });
    });
    ...
    describe ("burn", () => {
        it("Shouldn't be possible burn from zero address", async () => {
            const amount = 1;
            await expect(erc20MinterBurnableContract.connect(deployer).burn(constants.AddressZero, amount))
                .to.be.revertedWith("account shouldn't be zero");
        });

        it("Shouldn't be possible burn zero amount", async () => {
            const amount = 0;
            await expect(erc20MinterBurnableContract.connect(deployer).burn(accounts[0].address, amount))
                .to.be.revertedWith("amount shouldn't be zero");
        });
        ...
        it("Should be change balance", async () =>{
            ...
            await erc20MinterBurnableContract.connect(deployer).mint(accounts[0].address, mintAmount);
            await erc20MinterBurnableContract.connect(deployer).burn(accounts[0].address, burnAmount);
